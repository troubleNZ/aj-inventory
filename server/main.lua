...
function IsVehicleOwned(plate)
    local result = MySQL.Async.fetchScalar('SELECT 1 from player_vehicles WHERE plate = ?', {plate})
...
function GetStashItems(stashId)
	local items = {}
	local result = MySQL.Async.fetchScalar('SELECT items FROM stashitems WHERE stash = ?', {stashId})
	if result then
...
function GetOwnedVehicleItems(plate)
	local items = {}
	local result = MySQL.Async.fetchScalar('SELECT items FROM trunkitems WHERE plate = ?', {plate})
	if result then
...
function GetOwnedVehicleGloveboxItems(plate)
	local items = {}
	local result = MySQL.Async.fetchScalar('SELECT items FROM gloveboxitems WHERE plate = ?', {plate})
	if result then
		...
		if amount > 0 then
			if itemData ~= nil then
				...
				elseif itemData["name"] == "weaponlicense" then
					...
					info.lastname = Player.PlayerData.charinfo.lastname
					info.birthdate = Player.PlayerData.charinfo.birthdate
					info.type = "Lethal Weapons License"
				elseif itemData["type"] == "weapon" then
					amount = 1
					info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
				elseif itemData["name"] == "harness" then
					info.uses = 20
				elseif itemData["name"] == "cigarettes" then
                    info.uses = 20
				elseif itemData["name"] == "paintcan" then
                    info.uses = 3
				elseif itemData["name"] == "petfood" then
                    info.uses = 20
