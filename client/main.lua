    ...
    if not isCrafting and not inInventory then
        QBCore.Functions.GetPlayerData(function(PlayerData)
            if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
                ...
                --local DonoVendingMachine    = "donator"
                -- Is Player In Vehicle

                if IsPedInAnyVehicle(ped) then
                    local vehicle = GetVehiclePedIsIn(ped, false)
                    CurrentGlovebox = QBCore.Functions.GetPlate(vehicle)
                    --CurrentGlovebox = GetVehicleNumberPlateTextIndex(vehicle)
                    curVeh = vehicle
                    CurrentVehicle = nil
                else
                    local vehicle = QBCore.Functions.GetClosestVehicle()
                    if vehicle ~= 0 and vehicle ~= nil then
                        ...
                        if #(pos - trunkpos) < 2.0 and not IsPedInAnyVehicle(ped) then
                            if GetVehicleDoorLockStatus(vehicle) < 2 then
                                CurrentVehicle = QBCore.Functions.GetPlate(vehicle)
                                --CurrentVehicle = GetVehicleNumberPlateTextIndex(vehicle)
                                curVeh = vehicle
                                CurrentGlovebox = nil
                            else
                ...
                elseif DonoVendingMachine ~= nil then
                    local ShopItems = {}
                    ShopItems.label = "Supporters Shop"
                    ShopItems.items = Config.VendingItemDono
                    ShopItems.slots = #Config.VendingItemDono
                    TriggerServerEvent("inventory:server:OpenInventory", "shop", "Vendingdonoshop_"..math.random(1, 99), ShopItems)
                else
...
--third eye receiver
RegisterNetEvent("qb-shops:client:openVending")
AddEventHandler("qb-shops:client:openVending", function()
    local VendingMachine = GetClosestVending()
...
--dono shop cafe
RegisterNetEvent("qb-shops:client:openDonoVending")
AddEventHandler("qb-shops:client:openDonoVending", function()
    local VendingMachine = 'exclusive'
    if VendingMachine ~= nil then
        local ShopItems = {}
        ShopItems.label = "Supporter Bar"
        ShopItems.items = Config.VendingItemDono
        ShopItems.slots = #Config.VendingItemDono
        TriggerServerEvent("inventory:server:OpenInventory", "shop", "exclusiveshop_"..math.random(1, 99), ShopItems)
    end
end)