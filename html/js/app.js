            ...
            if ((ItemData.name).split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    // if (ItemData.info.attachments !== null && ItemData.info.attachments !== undefined && ItemData.info.attachments.length > 0) {
                        $(".inv-options-list").append('<center><div style="background-color: #468786;" class="inv-option-item" id="weapon-attachments"><p>ATTACHMENTS</p></div></center>');
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html("<p>" + itemData.info.uses + " cigarettes left.</p>");
        } else if (itemData.name == "paintcan") {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html("<p>" + itemData.info.uses + " uses left.</p>");
...
function handleDragDrop() {
    $(".item-drag").draggable({
        ...
        stop: function() {
            ...
            //$(this).css("background", "rgba(0, 0, 0, 0.123)");    background-color: rgba(1, 12, 22, 0.5)
            $(this).css("background", "rgba(1, 12, 22, 0.5)");
    ...
    Inventory.ToggleHotbar = function(data) {
        if (data.open) {
            ...
            for(i = 1; i < 6; i++) {
                ...
                var elem = '<div class="z-hotbar-item-slot" data-zhotbarslot="'+i+'"> <div class="quarter-ellipse3"><div class="z-hotbar-item-slot-key"><p>'+i+'</p></div></div><div class="z-hotbar-item-slot-img"></div><div class="z-hotbar-item-slot-label"><p>&nbsp;</p></div></div>'
                $(".z-hotbar-inventory").append(elem);
            }
            var elem = '<div class="z-hotbar-item-slot" data-zhotbarslot="41"> <div class="quarter-ellipse3"><div class="z-hotbar-item-slot-key"><p>6 <i class="fas fa-lock"></i></p></div></div><div class="z-hotbar-item-slot-img"></div><div class="z-hotbar-item-slot-label"><p>&nbsp;</p></div></div>'
            $(".z-hotbar-inventory").append(elem);
            $.each(data.items, function(i, item){
                if (item != null) {
                    ...
                    if (item.slot == 41) {
                        $(".z-hotbar-inventory").find("[data-zhotbarslot=" + item.slot + "]").html('<div class="quarter-ellipse3"><div class="z-hotbar-item-slot-key"><p>6 <i class="fas fa-lock"></i></p></div></div><div class="z-hotbar-item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="z-hotbar-item-slot-amount"><p>' + item.amount + ' (' + ((item.weight * item.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                    } else {
                        $(".z-hotbar-inventory").find("[data-zhotbarslot=" + item.slot + "]").html('<div class="quarter-ellipse3"><div class="z-hotbar-item-slot-key"><p>' + item.slot + '</p></div></div><div class="z-hotbar-item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="z-hotbar-item-slot-amount"><p>' + item.amount + ' (' + ((item.weight * item.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                    }
        ...
        window.addEventListener('message', function(event) {
            switch(event.data.action) {
                ...
                case "toggleHotbar":
                    ...
                    break;
                case "RobMoney":
                    $(".inv-options-list").append('<center><div style="background-color: #468786;" class="inv-option-item" id="rob-money"><p>Take Cash</p></div>');
